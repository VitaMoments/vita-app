...
const Registration: React.FC = () => {
  ...
  function validatePassword(pw: string, confirmPw: string): string[] {
    const errors: string[] = [];
    if (pw.length < 6)
      errors.push("Wachtwoord moet minimaal 6 tekens lang zijn.");
    if (!/[A-Z]/.test(pw))
    ...
    if (!/[^\w\s]/.test(pw))
      errors.push(
        "Wachtwoord moet minstens 1 leesteken bevatten (bijv. !, ?, .).",
      );
    if (confirmPw.length > 0 && pw !== confirmPw) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(
        err.response?.data?.message || "Er is iets misgegaan bij registratie",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className={styles.container}>
      ...
      <p className={styles.subtitle}>
        Maak een account aan om toegang te krijgen tot je persoonlijke vita
        account.
      </p>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Button
          type="submit"
          disabled={loading || (pwError?.length ?? 0) > 0 || !hasBothPasswords}
        >
      ...
      <div className={styles.link}>
        <Link to="/login">Al een account? Log hier in</Link>
      </div>
    </div>
  );
};
